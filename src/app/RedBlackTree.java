...
// References from https://github.com/Arsenalist/Red-Black-Tree-Java-Implementation
...
public class RedBlackTree<T extends Comparable<T>> {
	...
	// @param: two arguments with arg1 < arg2
	// @return: List of nodes in range [arg1..arg2]
	@SuppressWarnings("unchecked")
	public List<Integer> searchInRange(int arg1, int arg2) {
		List<Integer> arr = new ArrayList<Integer>();
		return searchInRange((RedBlackNode<Integer>) root, arg1, arg2, arr);
	}

	// @param: RedBlackNode node, range [low..high], list to store nodes found in range [low..high]
	// @return: list of nodes found
	private List<Integer> searchInRange(RedBlackNode<Integer> node, int low, int high, List<Integer> arr) {
		if (node.key == null) {
			return arr;
		}	
		if (low < node.key) {
			searchInRange(node.left, low, high, arr);
		}
		if (low <= node.key &&  high >= node.key) {
			arr.add(node.key);
		}
		if (high > node.key) {
			searchInRange(node.right, low, high, arr);
		} 
		return arr;
	}
